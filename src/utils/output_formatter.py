...
# URL 域名 → 招聘平台中文名称 映射表
# 与 job_searcher_node.py 中的 allowed_sites 顺序保持一致
DOMAIN_TO_SOURCE = {
    "nowcoder.com":   "牛客网",
    "zhipin.com":     "BOSS直聘",
    "shixiseng.com":  "实习僧",
...
def _infer_source_from_url(url: str) -> str | None:
    """
    从 URL 中解析域名，匹配 DOMAIN_TO_SOURCE 映射表，
    返回对应的招聘平台中文名称。如果无法匹配则返回 None。
    """
    try:
        host = urlparse(url).netloc.lower()
    except Exception:
        return None
    for domain, source_name in DOMAIN_TO_SOURCE.items():
        if domain in host:
            return source_name
    return None
...
def format_and_save_jobs(jobs: List[Dict], output_dir: str = ".") -> tuple:
    ...
    for job in jobs:         # 遍历每一个岗位字典
        ...
        # 6. 【source 字段硬编码修正】
        # 不依赖 LLM 从页面内容提取，而是直接通过 URL 域名判断来源平台，
        # 避免 LLM 幻觉导致的招聘平台名称不一致问题。
        inferred_source = _infer_source_from_url(url)
        if inferred_source:
            cleaned_job["source"] = inferred_source

        # 7. 将这颗"好果子"放进我们最终的篮子里
